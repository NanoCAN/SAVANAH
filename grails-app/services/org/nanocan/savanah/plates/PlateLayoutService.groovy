...
class PlateLayoutService {
    ...
    def createWellLayouts(PlateLayout plateLayout) {
        ...
        //get config
        int batchSize = grailsApplication.config.savanah.jdbc.batchSize?:200
        boolean useGroovySql = grailsApplication.config.savanah.jdbc.groovySql.toString().toBoolean()
    ...
    def updateWellProperties(params, wellProp, plateLayout){
        ...
        params.each {key, value ->
            if (value != "") {
                ...
                else well.properties[wellProp] = grailsApplication.getArtefactByLogicalPropertyName("Domain", wellProp).clazz.get(value as Long)
