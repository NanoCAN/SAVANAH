	...
	<body>
		...
		<div id="list-numberOfCellsSeeded" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
                        <g:sortableColumn property="number" title="${message(code: 'numberOfCellsSeeded.number.label', default: 'Number')}" />

						<g:sortableColumn property="comments" title="${message(code: 'numberOfCellsSeeded.comments.label', default: 'Comments')}" />
				...
				<g:each in="${numberOfCellsSeededInstanceList}" status="i" var="numberOfCellsSeededInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${numberOfCellsSeededInstance.id}">${fieldValue(bean: numberOfCellsSeededInstance, field: "number")}</g:link></td>
					
						<td>${fieldValue(bean: numberOfCellsSeededInstance, field: "comments")}</td>
